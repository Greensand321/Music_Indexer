...
class GraphWorkspace(WorkspaceBase):
    ...
    def _on_open_graph(self) -> None:
        try:
            ...
            self._graph_win = QtWidgets.QDialog(self)
            self._graph_win.setWindowTitle("Visual Music Graph")
            self._graph_win.resize(1000, 700)
            layout = QtWidgets.QVBoxLayout(self._graph_win)
            panel = ClusterGraphPanel(
                library_path=self._library_path,
                parent=self._graph_win
            )
            layout.addWidget(panel)
            self._graph_win.show()
        except ImportError:
            self._status_lbl.setText("cluster_graph_panel not available.")
            self._log("cluster_graph_panel.py not found in sys.path.", "error")
