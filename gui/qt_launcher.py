...
class QtPreviewWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self._tool_workspace_expanded = 900
    ...
    def _build_tool_workspace(self) -> QtWidgets.QWidget:
        ...
        tabs.addTab(self._build_indexer_tab(), "Indexer")
        tabs.addTab(self._build_duplicate_finder_tab(), "Duplicate Finder")
    ...
    def _build_indexer_tab(self) -> QtWidgets.QWidget:
        widget = QtWidgets.QWidget()
        layout = QtWidgets.QVBoxLayout(widget)
        layout.setContentsMargins(8, 8, 8, 8)
        layout.setSpacing(12)
        ...
        stepper.setStyleSheet("background: #eff6ff; border-radius: 10px;")
        stepper_layout = QtWidgets.QHBoxLayout(stepper)
        stepper_layout.setContentsMargins(12, 10, 12, 10)

        step_label = QtWidgets.QLabel("Preview → Review Report → Execute Changes")
        step_label.setStyleSheet("font-weight: 600; color: #1d4ed8;")
        ...
        toggles = QtWidgets.QGroupBox("Main Options")
        toggles.setStyleSheet("QGroupBox { font-weight: 600; }")
        toggles_layout = QtWidgets.QVBoxLayout(toggles)
        self._dry_run_toggle = QtWidgets.QCheckBox("Dry Run (preview only)")
        self._dry_run_toggle.setChecked(True)
        self._cross_album_toggle = QtWidgets.QCheckBox("Enable Cross-Album Scan (Phase 3)")
        self._playlist_toggle = QtWidgets.QCheckBox("Create Playlists")
        self._playlist_toggle.setChecked(True)
        toggles_layout.addWidget(self._dry_run_toggle)
        toggles_layout.addWidget(self._cross_album_toggle)
        toggles_layout.addWidget(self._playlist_toggle)
        toggles_layout.addStretch(1)

        actions = QtWidgets.QGroupBox("Primary Actions")
        actions.setStyleSheet("QGroupBox { font-weight: 600; }")
        actions_layout = QtWidgets.QVBoxLayout(actions)
        start_button = QtWidgets.QPushButton("Start Indexer")
        start_button.setStyleSheet(
            "padding: 8px 12px; border-radius: 8px; background: #1d4ed8; color: white;"
        )
        open_unsorted = QtWidgets.QPushButton("Open “Not Sorted” Folder")
        cancel_button = QtWidgets.QPushButton("Cancel")
        cancel_button.setStyleSheet("padding: 6px 12px; border-radius: 8px; background: #e2e8f0;")
        actions_layout.addWidget(start_button)
        actions_layout.addWidget(open_unsorted)
        actions_layout.addWidget(cancel_button)
        actions_layout.addStretch(1)

        options_row.addWidget(toggles, 2)
        options_row.addWidget(actions, 1)
        layout.addLayout(options_row)

        advanced = QtWidgets.QGroupBox("Advanced Options")
        advanced.setCheckable(True)
        advanced.setChecked(False)
        advanced.setStyleSheet("QGroupBox { font-weight: 600; }")
        advanced_layout = QtWidgets.QFormLayout(advanced)

        flush_cache = QtWidgets.QCheckBox("Flush Cache")
        max_workers = QtWidgets.QSpinBox()
        max_workers.setRange(1, 32)
        max_workers.setValue(8)
        advanced_layout.addRow(flush_cache)
        advanced_layout.addRow("Max Workers", max_workers)
        layout.addWidget(advanced)

        report_row = QtWidgets.QHBoxLayout()
        report_button = QtWidgets.QPushButton("Open Preview Report")
        report_button.setStyleSheet(
            "padding: 6px 12px; border-radius: 8px; background: #e2e8f0;"
        )
        ...
        for title, desc in (
            ...
            ("Phase B — Planning", "Build rename plan and preview report."),
            ("Phase C — Cross-album checks", "Validate duplicates and cross-album matches."),
        ):
            phase = QtWidgets.QFrame()
            phase_layout = QtWidgets.QVBoxLayout(phase)
            header = QtWidgets.QLabel(title)
            header.setStyleSheet("font-weight: 600; color: #0f172a;")
            sub = QtWidgets.QLabel(desc)
            sub.setStyleSheet("color: #64748b; font-size: 11px;")
            bar = QtWidgets.QProgressBar()
            bar.setValue(0)
            phase_layout.addWidget(header)
            phase_layout.addWidget(sub)
            phase_layout.addWidget(bar)
            progress_layout.addWidget(phase)

        status = QtWidgets.QLabel("Status: Ready to preview.")
        status.setToolTip("Dry Run will generate Docs/MusicIndex.html and Docs/indexer_log.txt.")
        status.setStyleSheet("color: #475569; font-size: 12px;")
        progress_layout.addWidget(status)
        layout.addWidget(progress_group)

        log_group = QtWidgets.QGroupBox("Run Log")
        log_group.setStyleSheet("QGroupBox { font-weight: 600; }")
        log_layout = QtWidgets.QVBoxLayout(log_group)
        log_area = QtWidgets.QTextEdit()
        log_area.setReadOnly(True)
        log_area.setPlaceholderText("Detailed output will appear here.")
        log_layout.addWidget(log_area)
        layout.addWidget(log_group)

        output_note = QtWidgets.QLabel(
            "Outputs: Docs/MusicIndex.html • Docs/indexer_log.txt • Manual Review/ for missing metadata"
        )
        output_note.setStyleSheet("color: #64748b; font-size: 11px;")
        layout.addWidget(output_note)
        layout.addStretch(1)
        return widget

    def _build_duplicate_finder_tab(self) -> QtWidgets.QWidget:
        widget = QtWidgets.QWidget()
        layout = QtWidgets.QVBoxLayout(widget)
        layout.setContentsMargins(8, 8, 8, 8)
        layout.setSpacing(12)

        heading = QtWidgets.QLabel("Duplicate Finder Launcher")
        heading.setStyleSheet("font-size: 14px; font-weight: 600; color: #0f172a;")
        layout.addWidget(heading)

        description = QtWidgets.QLabel(
            "This opens the full Duplicate Finder workflow for scan → preview → execute."
        )
        description.setWordWrap(True)
        description.setStyleSheet("color: #475569;")
        layout.addWidget(description)

        last_scan = QtWidgets.QLabel("Last scan: Today 2:14 PM • 36 groups found")
        last_scan.setStyleSheet("color: #1d4ed8; font-weight: 600;")
        layout.addWidget(last_scan)

        buttons = QtWidgets.QHBoxLayout()
        open_button = QtWidgets.QPushButton("Open Duplicate Finder")
        open_button.setStyleSheet(
            "padding: 8px 12px; border-radius: 8px; background: #1d4ed8; color: white;"
        )
        preview_button = QtWidgets.QPushButton("Open Last Preview")
    ...
    def _set_focus_state(self, focused: bool) -> None:
        ...
        for widget in self._fade_targets:
            ...
            fade_anim.setEndValue(0.0 if focused else 1.0)
            animation_group.addAnimation(fade_anim)
            widget.setEnabled(not focused)

        if focused:
            animation_group.finished.connect(
                lambda: [widget.setVisible(False) for widget in self._fade_targets]
            )

        self._focus_animation = animation_group
