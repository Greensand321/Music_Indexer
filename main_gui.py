    ...
    def _gather_tracks(
        ...
        *,
        ...
        if flush_after_compute:
            ...
            pre_flush_tracks_map = dict(tracks_map)
            ...
            missing_paths = [
                path
                for path in sorted_paths
                if path not in tracks_map and path in pre_flush_tracks_map
            ]
            if missing_paths:
                if log_callback:
                    log_callback(
                        "Fingerprint cache refresh missed entries; retrying cache write "
                        f"for {len(missing_paths)} tracks."
                    )
                for path in missing_paths:
                    payload = pre_flush_tracks_map.get(path, {})
                    if not store_fingerprint(
                        path,
                        db_path,
                        None,
                        ...
                        if log_callback:
                            log_callback(f"! Cache retry failed for {path}")
                flush_fingerprint_writes(db_path)
                (
                    tracks_map,
                    _pending_paths,
                ...
                if unresolved_paths:
                    if log_callback:
                        log_callback(
                            "Fingerprint cache refresh still missing entries; "
                            f"falling back to in-memory fingerprints for {len(unresolved_paths)} tracks."
                        )
                    for path in unresolved_paths:
                        tracks_map[path] = pre_flush_tracks_map[path]
                    missing_count = max(0, missing_count - len(unresolved_paths))
